...
public:
  WindowedOrderBook(Price tickSize, Price expectedDeviation,
                    std::pmr::memory_resource *mem)
      : _tickSize(tickSize), _invTickSize(1.0 / tickSize.toDouble()),
        _windowSize(static_cast<size_t>(std::ceil(
            (expectedDeviation.toDouble() * 2) / tickSize.toDouble()))),
        _halfWindowSize(_windowSize / 2), _centerPrice(Price(0)),
        _basePrice(Price(0)), _bids(_windowSize, BookSide::Side::Bid, mem),
  ...
  void applyBookUpdate(const BookUpdateEvent &update) override {
    ...
    if (update.type == BookUpdateType::SNAPSHOT) {
      ...
      for (const auto &lvl : update.bids) {
        ...
        if (offset >= 0 &&
            offset < static_cast<int64_t>(_tickSize.raw() * _windowSize)) {
  ...
  size_t priceToIndex(Price price) const {
    return static_cast<size_t>((price.raw() - _basePrice.raw()) /
                               _tickSize.raw());
  }
  ...
  std::optional<Price> bestBid() const override {
    ...
    return idx.has_value() ? std::optional<Price>{indexToPrice(*idx)}
                           : std::nullopt;
  }
  ...
  void printBook(std::size_t depth = 10) const {
    std::scoped_lock lock(_mutex);
    std::cout << "=== WindowedOrderBook Snapshot (center="
              << _centerPrice.toDouble() << ") ===\n";
    ...
    for (int i = static_cast<int>(_windowSize) - 1; i >= 0; --i) {
      ...
      if (lvl.raw() > 0) {
        std::cout << "  " << indexToPrice(i).toDouble() << " x "
                  << lvl.toDouble() << "\n";
      }
    }
  ...
  void shiftWindow(Price newPrice) {
    int64_t newBaseRaw =
        static_cast<int64_t>(std::round(
            (newPrice.toDouble() - _tickSize.toDouble() * _halfWindowSize) *
            _invTickSize)) *
        _tickSize.raw();

    int shift = static_cast<int>(
        std::round((newBaseRaw - _basePrice.raw()) / _tickSize.raw()));

    if (_centerPrice.raw() == 0 ||
